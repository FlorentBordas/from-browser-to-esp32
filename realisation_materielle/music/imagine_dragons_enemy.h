...
int imagine_dragons_enemy_melody[] = {
  NOTE_B4, REST,
...
int imagine_dragons_enemy_durations[] = {
  4, 2,
  4, 8, 4, 8, 4, 2, 8,
...
void play_imagine_dragons_enemy(int pin)
{
  int size = sizeof(imagine_dragons_enemy_durations) / sizeof(int);

  for (int note = 0; note < size; note++) {
    //to calculate the note duration, take one second divided by the note type.
    //e.g. quarter note = 1000 / 4, eighth note = 1000/8, etc.
    int duration = 1000 / imagine_dragons_enemy_durations[note];
    tone(pin, imagine_dragons_enemy_melody[note], duration);
