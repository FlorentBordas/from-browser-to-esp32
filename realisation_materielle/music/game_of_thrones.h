...
void play_game_of_thrones(int pin)
{
  int size = sizeof(game_of_thrones_durations) / sizeof(int);

  for (int note = 0; note < size; note++) {
    //to calculate the note duration, take one second divided by the note type.
    //e.g. quarter note = 1000 / 4, eighth note = 1000/8, etc.
    int duration = 1000 / game_of_thrones_durations[note];
    tone(pin, game_of_thrones_melody[note], duration);
